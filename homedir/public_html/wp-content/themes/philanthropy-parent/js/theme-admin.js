jQuery(document).ready(function($) {
    ...
    if ( typeof fwEvents !== 'undefined') {
        ...
        fwEvents.on('fw:options:init', function (data) {
            ...
            // fw gallery type 5
            var gallery_gap        = jQuery('#fw-backend-option-fw-edit-options-modal-fw_gallery_gap');
            var gallery_image_type = jQuery('#fw-backend-option-fw-edit-options-modal-fw_gallery_img_type');
            if ( gallery_image_type.length > 0 ) {
                var gallery_general_type = jQuery('#fw-edit-options-modal-gallery_style .image_picker_selector .thumbnail.selected img');
                var src = gallery_general_type.attr('src');
                if( src.indexOf("fw-gallery-5") > -1 ) {
                    gallery_image_type.show();
                    fw_hide_gallery_options(src, gallery_gap);
                }
            }

            // fw gallery gap
            if ( gallery_gap.length > 0 ) {
                var src = jQuery('#fw-edit-options-modal-gallery_style').find('.image_picker_selector .thumbnail.selected img').attr('src');
                fw_hide_gallery_options(src, gallery_gap);
            }

            jQuery('#fw-edit-options-modal-gallery_style').on('click', '.image_picker_selector .thumbnail', function(){
...
function fw_hide_gallery_options(src, gallery_gap){
	"use strict";
